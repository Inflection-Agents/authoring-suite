// Restate vs Temporal: what the next size costs (v2) — Typst evaluation build.
// Source prose: drafts/restate-vs-temporal-v2-6pager.md (section titles verbatim).
// [CITE key] markers become numbered endnotes (#c(n)); URL map matches the HTML build.
// Figures: inline SVGs extracted from figures/figN-*.html, one per matching section.

#set page(
  paper: "a4",
  margin: (top: 2cm, bottom: 2.4cm, left: 2.2cm, right: 2.2cm),
  footer: context [
    #set text(font: "Helvetica Neue", size: 8pt, fill: rgb("#4A6275"))
    #line(length: 100%, stroke: 0.5pt + rgb("#C8C4BB"))
    #v(2pt)
    #grid(
      columns: (1fr, auto),
...
#set text(font: "Georgia", size: 10.5pt, fill: rgb("#1B2B3C"))
#set par(justify: true, spacing: 0.95em, leading: 0.68em)
#show heading: set text(font: "Helvetica Neue", fill: rgb("#1B2B3C"))
#show heading.where(level: 1): set text(size: 22pt, weight: 700)
#show heading.where(level: 2): it => block(
  above: 1.5em, below: 0.7em,
  text(size: 12pt, weight: 700)[#it.body],
)
#show link: set text(fill: rgb("#3A5F8B"))
#show figure: set text(font: "Helvetica Neue", size: 8.5pt, fill: rgb("#1B2B3C"))

// Numbered endnote reference (notes listed under "Notes" at the end).
#let c(n) = super(size: 7.5pt)[#n]

#block[
  #set par(justify: false, spacing: 0.55em)
  #text(font: "Helvetica Neue", size: 9.5pt, weight: 700, fill: rgb("#967A3A"))[EVALUATION BUILD · TYPST]
  #heading(level: 1)[Restate vs Temporal: what the next size costs (v2)]
  #text(style: "italic")[Audience: teams running Temporal today and comfortable with it. This paper meets that team where it stands and asks what each engine charges when workflows get longer and releases keep shipping.]
]
#v(0.2em)
#line(length: 100%, stroke: 1pt + rgb("#967A3A"))
...
#figure(
  image("fig1-tax-base.svg", width: 100%),
  caption: [Length is the tax base: per-workflow history cost grows with workflow lifetime.],
  kind: image,
  supplement: [Figure],
)

So length is the tax base this whole paper measures#c(1). The question is never whether Temporal runs your current workload. It runs it. The question is what each engine charges when that workload doubles in length and ships another release mid-flight, which means the rest of this paper reads as a bill, not a review.
...
The cost is visible in the source. Temporal's own blog on worker versioning says patching “adds clutter to your Workflow source files over time”#c(2). Each marker stays because some old execution may still replay it. For example, a file that began as forty clean lines carries three version branches a year later, each with its own comment explaining which quarter it belongs to.
...
History in Temporal is sharded (partitioned across a fixed shard count set at cluster creation; the default most teams meet first is 512 shards). That number fixes how history ownership spreads across the cluster. It is not a dial. Changing it requires a database reset#c(3), which means export, rebuild, and reimport, planned like a migration with its own runbook and rollback page.

This bites exactly when growth feels good. Traffic doubles. Latency on history writes climbs. The scaling guide names the cause directly: the persistence backend becomes the bottleneck#c(1). The team opens the config expecting a larger number and finds a rebuild instead.
...
#figure(
  image("fig2-deployment-shapes.svg", width: 100%),
  caption: [Deployment shapes side by side: fixed shard count versus versioned deployments.],
  kind: image,
  supplement: [Figure],
)
...
Histories have a ceiling. The Continue-As-New docs set it plainly: 51,200 events or 50 MB per execution as a hard limit, with a warning at 10,240 events#c(4). Payloads add their own walls nearby: 2 MB per payload and 4 MB per gRPC message#c(5). Long workflows hit these walls by succeeding, one event at a time.
...
#figure(
  image("fig3-continuation-chain.svg", width: 100%),
  caption: [Continue-As-New chain: one long workflow becomes many short executions the team must link.],
  kind: image,
  supplement: [Figure],
)
...
Restate inverts the delivery path. Temporal workers poll task queues; Restate pushes work to registered deployments over a bidirectional SDK connection (a persistent channel the SDK opens to the runtime)#c(6). Registration is explicit: a GET /services call on the Admin API returns each service with its handlers, deployment\_id, and revision#c(7). The runtime knows exactly which binary serves which handler at which revision.

That registry entry is what makes releases boring. A new deployment registers under a new deployment\_id. In-flight invocations finish on the old revision. New invocations route to the new one. No markers accumulate in handler code because the old code keeps running until its work drains, which means the release history lives in the deployment list instead of in branches inside the workflow file.

The packaging story matches. Restate ships as a single binary in converged mode (runtime, metadata store, and user-space services colocated for small setups)#c(8), so the team testing the version-drain behavior on a laptop runs the same binary shape that stages it. Push replaces the poll loop. Versioning replaces the marker.
...
#figure(
  image("fig4-failure-timeline.svg", width: 100%),
  caption: [Failure timeline, two lanes: routing back to a draining revision versus replaying branches.],
  kind: image,
  supplement: [Figure],
)
...
#list(
  marker: [#1.],
  link("https://dev.to/temporalio/scaling-temporal-the-basics-31l5")[Temporal scaling guide — dev.to/temporalio/scaling-temporal-the-basics-31l5],
)
#list(
  marker: [#2.],
  link("https://temporal.io/blog/automated-worker-versioning-with-github-actions")[Temporal worker versioning — temporal.io/blog/automated-worker-versioning-with-github-actions],
)
#list(
  marker: [#3.],
  link("https://medium.com/@yedidya.s/from-step-functions-to-temporal-on-eks-durable-workflows-at-scale-without-breaking-the-bank-02c23e837188")[Temporal shard configuration — medium.com/\@yedidya.s/from-step-functions-to-temporal-on-eks…],
)
#list(
  marker: [#4.],
  link("https://docs.temporal.io/develop/ruby/workflows/continue-as-new")[Temporal Continue-As-New — docs.temporal.io/develop/ruby/workflows/continue-as-new],
)
#list(
  marker: [#5.],
  link("https://docs.temporal.io/troubleshooting/blob-size-limit-error")[Temporal payload limits — docs.temporal.io/troubleshooting/blob-size-limit-error],
)
#list(
  marker: [#6.],
  link("https://restate.dev/blog/building-a-modern-durable-execution-engine-from-first-principles")[Restate runtime — restate.dev/blog/building-a-modern-durable-execution-engine-from-first-principles],
)
#list(
  marker: [#7.],
  link("https://docs.restate.dev/admin-api/service/list-services")[Restate Admin API — docs.restate.dev/admin-api/service/list-services],
)
#list(
  marker: [#8.],
  link("https://restate.dev/blog/building-a-modern-durable-execution-engine-from-first-principles")[Restate converged mode — restate.dev/blog/building-a-modern-durable-execution-engine-from-first-principles],
)
#list(
  marker: [#9.],
  link("https://docs.restate.dev/services/invocation/http")[Restate idempotency — docs.restate.dev/services/invocation/http],
)
