---
name: diagram-design
description: Create branded architecture, zoom (overview + detail), contrast (before/after), lineage (provenance and impact), composition (catalog and assembly), deployment (nested infra + runs-on), IT current-state, flowchart, sequence, state machine, code level (ER/data model or class structure), timeline, swimlane, quadrant, radar/spider, loop/flywheel, nested, tree, org chart, layer stack, Venn, pyramid/funnel, bar, line, Gantt and scatter charts, high-level, process, medallion, data flow, DP integration, or matrix (categorical grid, including a permissions-matrix preset) diagrams as standalone HTML/SVG/PNG. Redraw .drawio/.drawio.png/.drawio.svg or Mermaid .mmd sources at a chosen size/detail; onboard brand tokens from a website; add semantic patterns, callouts, accessible motion, or sketchy/hand-drawn styling.
license: MIT
metadata:
  version: "2.3"
---
...
Thirty-two visual types. Semantic patterns describe behavior independently; type references describe layout. Details load from `references/` only when selected.
...
Open [`references/style-guide.md`](references/style-guide.md) and check the default tokens. If they're still the shipped defaults (paper `#F5F2EC`, ink `#1B2B3C`, accent `#967A3A` gold), **pause and ask the user**:

> *"This is your first diagram in this project. The style guide is still at the default (Inflection Agents ivory + gold). Do you want to customize it to match your brand first? Options: (a) pull from your website URL, (b) extract from an installed skill, (c) extract from a local folder / design-system directory, (d) paste tokens manually, (e) proceed with the default for now."*
...
**Once the style guide has been customized** (or the user explicitly opted for default), skip this gate on subsequent runs. A simple way to detect customization: if the `accent` value in `style-guide.md` differs from `#967A3A`, assume custom.
...
- Gold is **editorial, not a flag.** 1–2 focal nodes per diagram. Using it on 5 nodes erases the signal.
...
semantic-pattern trigger table and the full visual-type guide (32 types). Kept out of this file
to stay under the byte cap as new types land; see its rules of thumb for tie-breaks and the
complexity-budget (§7) split-into-Zoom escape hatch.
...
| JetBrains Mono as blanket "dev" font | Mono is for *technical* content — ports, commands, URLs. Names go in DM Sans. |
...
| Gold on every "important" node | Gold is 1–2 editorial accents, not a signaling system |
...
| `node-white` | Backend / API / step node fill |
| `wash-2`…`wash-30` | The fixed-opacity fills in the treatment table below |
| `radius-node`, `radius-tag`, `radius-container` | Corner radius |

**Bind, don't inline.** Every colour and radius above is emitted as `var(--dd-<role>)`, with the
generated `:root` block from [`assets/skins/`](assets/skins/) pasted into the file's `<style>`. That
makes a reskin one block swap instead of one edit per attribute. For a translucent shade with no
named role, use `color-mix(in srgb, var(--dd-ink) 25%, transparent)`. Never write a raw hex.
...
Every node also carries a **mandatory type tag** (§6 node box pattern) — the primary kind channel,
readable as text. Diagrams whose nodes represent system components (architecture, data-flow,
dp-integration, high-level, it-state, swimlane, sequence) add a **corner notch** as the secondary,
distance-legible channel: 0/1/2 clipped corners for runs/holds/outside-our-control. See
[`primitive-notch.md`](references/primitive-notch.md) — it does not apply to types with their own
kind vocabulary (flowchart, state, tree, er, org-chart) or no kind concept at all (charts, pyramid,
venn, quadrant, timeline).

| Type | Fill | Stroke | Notch |
|---|---|---|---|
| **Focal** (1–2 max) | `accent-tint` | `accent`, stroke-width **1.6** | inherits base kind, else 0 |
| **Backend / API / Step** | white | `ink` | 0 |
| **Store / State** | `ink @ 0.05` | `muted` | 1 |
| **External / Cloud** | `ink @ 0.03` | `ink @ 0.30` | 2 |
| **Input / User** | `muted @ 0.10` | `soft` | 2 |
| **Optional / Async** | `ink @ 0.02` | `ink @ 0.20` dashed `4,3` | inherits base kind |
| **Security / Boundary** | `accent @ 0.05` | `accent @ 0.50` dashed `4,4` | inherits base kind, else 2 |

### Typography (summary — full spec in style-guide.md)

- **Title** — Cormorant Garamond, 1.75rem, 400 — H1 only
...
- **Editorial aside** — Cormorant Garamond *italic*, 14px — callouts only

**Mono is for technical content.** Names are DM Sans. Page title is Cormorant Garamond. Italic Cormorant Garamond is reserved for annotation callouts. Geist Mono is a disclosed fallback — the brand ships no monospace. Never JetBrains Mono as a blanket "dev" font.
...
```svg
<rect width="100%" height="100%" fill="var(--dd-paper)"/>
```
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="1" cy="1" r="0.9" fill="color-mix(in srgb, var(--dd-ink) 10%, transparent)"/>
  </pattern>
</defs>
<rect width="100%" height="100%" fill="var(--dd-paper)"/>
...
<marker id="arrow" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="var(--dd-muted)"/>
</marker>
<marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="var(--dd-accent)"/>
</marker>
<marker id="arrow-link" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="var(--dd-link)"/>
</marker>
```
...
<rect x="X" y="Y" width="W" height="H" rx="6" class="dd-node" fill="var(--dd-paper)"/>
<!-- 2. Styled box -->
...
<!-- 4. Node name (DM Sans — human-readable) -->
<text x="CX" y="CY+2" fill="var(--dd-ink)" font-size="12" font-weight="600"
      font-family="'DM Sans', sans-serif" text-anchor="middle">Node Name</text>
<!-- 5. Technical sublabel (Geist Mono) -->
<text x="CX" y="CY+18" fill="var(--dd-muted)" font-size="9"
...
<rect x="MID_X-18" y="ARROW_Y-20" width="36" height="12" rx="2" class="dd-tag" fill="var(--dd-paper)"/>
<text x="MID_X" y="ARROW_Y-11" fill="var(--dd-soft)" font-size="8"
...
<line x1="30" y1="LEGEND_Y-8" x2="VIEWBOX_W-30" y2="LEGEND_Y-8"
      stroke="color-mix(in srgb, var(--dd-ink) 10%, transparent)" stroke-width="0.8"/>
<text x="30" y="LEGEND_Y+8" fill="var(--dd-muted)" font-size="8" font-family="'Geist Mono', monospace"
...
1. **Header** — eyebrow (Geist Mono), title (Cormorant Garamond), optional subtitle (DM Sans muted).
...
- `background: var(--dd-node-white)` (not paper — slight lift without shadow)
...
- [ ] Gold used on ≤2 elements? If more, which actually deserve focal status?
...
- [ ] Every arrow label has an opaque `fill="var(--dd-paper)"` rect behind it?
...
- [ ] Human-readable names in DM Sans, not Geist Mono?
- [ ] Technical sublabels (ports, commands, URLs) in Geist Mono?
- [ ] Page title in Cormorant Garamond?
- [ ] Annotation callouts (if any) in *italic* Cormorant Garamond? (see [primitive-annotation.md](references/primitive-annotation.md))
...
- Embedded CSS (no external except Google Fonts), including the generated `--dd-*` skin block inlined into `:root` — the skin is never linked, so the file stays self-contained and reskinnable
...
### Multiple diagrams describing the same system

When two or more diagrams must agree — a container-level overview and the component-level
detail it drills into, for example — load [`references/diagram-sets.md`](references/diagram-sets.md)
and declare a set via `<meta name="diagram:*">` tags. Opt-in only; a file with no `diagram:` meta
tags stays exactly as zero-ceremony as it is today.
