...
```svg
<!-- 1. Italic Cormorant Garamond text -->
<text x="904" y="36" fill="var(--dd-ink)" font-size="14" font-style="italic"
      font-family="'Cormorant Garamond', serif" text-anchor="end">no imports, no configuration</text>
<!-- 2. Dashed Bézier leader -->
<path d="M 820 44 Q 700 84 520 216" fill="none"
      stroke="color-mix(in srgb, var(--dd-ink) 40%, transparent)" stroke-width="1" stroke-dasharray="4,3"/>
<!-- 3. Landing dot -->
<circle cx="520" cy="216" r="2" fill="var(--dd-ink)"/>
```
...
| Intent | Text | Leader |
|---|---|---|
| Neutral aside | ink `var(--dd-ink)` | `color-mix(in srgb, var(--dd-ink) 40%, transparent)` |
| Focal / accent | gold `var(--dd-accent)` | `color-mix(in srgb, var(--dd-accent) 50%, transparent)` |
| Tertiary (muted) | muted `var(--dd-muted)` | `var(--dd-wash-30)` |
