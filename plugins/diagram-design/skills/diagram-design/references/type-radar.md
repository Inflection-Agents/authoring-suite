...
- **Axis labels:** one word per spoke (Jobs-minimal). DM Sans sans 11px weight 600. Place 16px outside the outer ring along the axis vector. Top/bottom = `text-anchor="middle"`; right side = `start`; left side = `end`.
...
- **Legend:** horizontal strip at the bottom (per the global rule). Swatch is a 16×8 rectangle (matches the polygon stroke+fill, not a circle), then the entity name. ~140px between entries. Optional italic tail on the right with the rationale (`"One gold. Position is the signal — color reserved for the recommended option."`).
...
<polygon points="500,96 622,201 585,356 415,356 363,196"
         fill="color-mix(in srgb, var(--dd-accent) 18%, transparent)" stroke="var(--dd-accent)" stroke-width="1.8"/>
```
...
|---|---|---|---|
| Focal | `accent` | `var(--dd-accent)` | `var(--dd-accent)` |
| 1 | `series-1` (sage) | `var(--dd-series-1)` | `var(--dd-series-1)` |
| 2 | `series-2` (dusty-blue) | `var(--dd-series-2)` | `var(--dd-series-2)` |
| 3 | `series-3` (mustard) | `var(--dd-series-3)` | `var(--dd-series-3)` |
| 4 | `series-4` (rust-brown) | `var(--dd-series-4)` | `var(--dd-series-4)` |
| 5 | `series-5` (slate) | `var(--dd-series-5)` | `var(--dd-series-5)` |
...
- **Mono-font axis labels.** Names go in DM Sans sans (the global rule). Mono is for technical sublabels only.
