...
- **Y-axis gridlines:** 4–6 horizontal lines at regular intervals. Stroke `color-mix(in srgb, var(--dd-ink) 8%, transparent)` (very faint), 0.8px. X-axis baseline at `color-mix(in srgb, var(--dd-ink) 25%, transparent)`, 1px.
- **Y-axis labels:** right-aligned Geist Mono 8px muted, at x=72 (8px left of the plot area).
- **X-axis labels:** centered below each bar, DM Sans sans 11px 600 for category names.
...
<rect x="X" y="Y" width="W" height="H" fill="var(--dd-paper)"/>
<!-- Bar body -->
<rect x="X" y="Y" width="W" height="H" fill="color-mix(in srgb, var(--dd-muted) 15%, transparent)" stroke="var(--dd-muted)" stroke-width="1"/>
<!-- Value label above bar -->
<text x="X+W/2" y="Y-8" fill="var(--dd-muted)" font-size="8" font-family="'Geist Mono', monospace" text-anchor="middle">VALUE</text>
```

Focal bar: replace fill with `color-mix(in srgb, var(--dd-accent) 12%, transparent)`, stroke with `var(--dd-accent)`, label fill with `var(--dd-accent)`.
