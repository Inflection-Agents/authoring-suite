...
focal_accent: "var(--dd-accent)"             # one color for all focal components (default = SKILL accent)
dark: false
```
...
Bars span the full zone width minus 16-px padding on each side. Bars marked `focal: true` use `bar_h_focal=56` and accent styling (fill `rgba(focal_accent, 0.08)`, stroke `focal_accent`). Non-focal bars use `bar_h_default=44` with muted styling (fill `var(--dd-wash-5)`, stroke `color-mix(in srgb, var(--dd-ink) 32%, transparent)`).
...
All side-column nodes use fixed `w=160 h=64`. Same fill / stroke pattern: fill `color-mix(in srgb, var(--dd-muted) 6%, transparent)`, stroke `var(--dd-soft)`, stroke-width 1.
...
|---|---|---|---|---|---|
| `primary` | `var(--dd-accent)` (focal_accent) | 1.4 | — | `arrow-accent` | Every edge whose endpoint is a `focal: true` component. Also every Trino → consumer edge (serve-flow rule). |
| `secondary` | `var(--dd-muted)` (muted) | 1.2 | — | `arrow` | Default for internal platform-component edges and source → platform edges that don't touch focal. |
| `federated` | `var(--dd-link)` (link-blue) | 1.0 | `4,3` | `arrow-link` | Federation queries (e.g., source DB → Trino). |
| `trigger` | `var(--dd-muted)` (muted) | 1.0 | `4,3` | `arrow` | Every edge originating from a `kind: bar` component (Airflow drops). Unlabelled. |
| `auth` | `var(--dd-accent)` | 1.2 | `5,4` | `arrow-accent` | Every edge from a footer node up to the zone bottom edge. **Never to a specific component.** |
...
<defs>
  ...
  <marker id="arrow-link"   markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="var(--dd-link)"/></marker>
...
| Muted | `var(--dd-muted)` | `var(--dd-muted)` |
| Accent | `var(--dd-accent)` | `var(--dd-accent)` |
| Link (federated) | `var(--dd-link)` | `var(--dd-link)` |
| Side-column fill | `color-mix(in srgb, var(--dd-muted) 6%, transparent)` | `color-mix(in srgb, var(--dd-ink) 6%, transparent)` |
| Side-column stroke | `var(--dd-soft)` | `color-mix(in srgb, var(--dd-ink) 30%, transparent)` |
| Zone fill | `var(--dd-wash-3)` | `color-mix(in srgb, var(--dd-ink) 4%, transparent)` |
| Zone stroke | `color-mix(in srgb, var(--dd-ink) 32%, transparent)` | `color-mix(in srgb, var(--dd-ink) 30%, transparent)` |
| Non-focal bar fill | `var(--dd-wash-5)` | `color-mix(in srgb, var(--dd-ink) 6%, transparent)` |
...
- **Per-tool color coding** (teal-NiFi, magenta-MinIO, yellow-Jupyter) inside the zone — collapses hierarchy; only the two focal accents earn gold, plus up to 2 custom colors on cross-cutting components (§4 cap).
