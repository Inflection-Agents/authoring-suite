...
- **Left label column:** x=20–200 (180px). Task names in DM Sans sans 11px 600. Phase labels as Geist Mono 7px eyebrows above each group.
...
- **Phase grouping:** a subtle zone rect (same pattern as architecture zone) behind each phase's rows, with an eyebrow label in the top-left margin. Use `var(--dd-wash-2)` fill, `color-mix(in srgb, var(--dd-ink) 10%, transparent)` stroke.
...
<rect x="X_start" y="ROW_Y+8" width="DURATION_PX" height="24" rx="4"
      fill="color-mix(in srgb, var(--dd-muted) 15%, transparent)" stroke="var(--dd-muted)" stroke-width="1"/>
<text x="X_start+8" y="ROW_Y+25" fill="var(--dd-ink)" font-size="10" font-weight="600"
      font-family="'DM Sans', sans-serif">Task name</text>

<!-- Focal task -->
<rect x="X_start" y="ROW_Y+8" width="DURATION_PX" height="24" rx="4"
      fill="color-mix(in srgb, var(--dd-accent) 12%, transparent)" stroke="var(--dd-accent)" stroke-width="1"/>
<text x="X_start+8" y="ROW_Y+25" fill="var(--dd-accent)" font-size="10" font-weight="600"
      font-family="'DM Sans', sans-serif">Key task</text>
```
