...
**Best for:** two independent dimensions varying at once, where the *combination* is the
insight — not a list, not a hierarchy, a grid. One type, two presets — see
[Which variant](#which-variant).

## Which variant

| If you're showing… | Use the |
|---|---|
| A general categorical grid — regions × products, teams × capabilities, anything where both axes are open-ended categories | **General preset** |
| Per-role / per-component access permissions for a data platform, one value per cell from a closed vocabulary | **Security-matrix preset** |

Both presets share the same spine: both axes explicitly labeled at their heads, cells holding
instances or values rather than prose, and the claim's region (a cell, row, column, or the
diagonal) visually marked rather than left for the reader to infer from a caption. The
security-matrix preset adds a closed cell-value vocabulary and a parametric input schema on top
of that shared spine — it retired from its own former type (`type-dp-security-matrix.md`) into
this one, at no cost to the type count, once it was clear its layout grammar and the general
matrix's were the same archetype.

---

## General preset

**Best for:** an open-ended categorical grid — regions × products, teams × capabilities —
where the reader's takeaway is a claim about one cell, row, column, or the diagonal.

### Layout conventions
- Both axes labeled at their heads, in plain text — no rotated or vertical text, ever.
- Cells hold **instances** (a small chip per item present) or values, never prose. A cell with
  two things present shows two small chips; a cell with none shows a deliberate empty-marker,
  never a blank that could read as an oversight.
- **The grid can hold with zero drawn lines.** Alternating row background tints plus small
  gaps between columns (negative space, not stroked rules) are enough structure for a 3×3 to
  4×4 grid — test whether your own grid needs ruled lines at all before adding them.
- **Mark the claim's region as one element**, not one accent per cell. A marked column spanning
  all its rows is a single tinted/stroked rect behind the whole column's cells, not three
  separately-accented cells — the latter blows the 2-accent budget immediately on anything
  larger than a 2-row grid.
- Empty cells: an em-dash, centered, muted. It must read as "deliberately not here," not "data
  missing."

### Anti-patterns
- Becoming a data table — if a reader starts studying individual cell text instead of taking in
  the pattern at a glance, the matrix has failed its own test.
- Axes that aren't actually independent — collapses half the grid's meaning.
- More than roughly 4×4 — split into two matrices instead.
- Unlabeled axes, or axis labels rotated to vertical text to save space.

### Examples
- `assets/example-matrix.html` — minimal light
- `assets/example-matrix-dark.html` — minimal dark
- `assets/example-matrix-full.html` — full editorial

---

## Security-matrix preset
...
This preset is **parametric** — the inputs schema in §1 drives every coordinate via the formulas in §2. The rule shape mirrors `type-medallion.md` / `type-process.md` / `type-data-flow.md` so the focal rule, color override, dark mode, and reproducibility checklist read identically across types.
...
| `rw` | `var(--dd-node-white)` | `ink @ 0.12` | `ink` | 400 |
...
|---|---|
| Banner fill | `C` |
| Role name + code text | `var(--dd-node-white)` if `C` is dark (luminance ≤ 0.5), else `ink` |
...
| R/W fill | `var(--dd-node-white)` | `paper @ 0.06` |
