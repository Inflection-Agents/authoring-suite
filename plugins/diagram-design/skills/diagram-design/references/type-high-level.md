...
Fills alternate `var(--dd-ink)` / `color-mix(in srgb, var(--dd-ink) 90%, var(--dd-paper))` (light) or `var(--dd-paper-2)` / `color-mix(in srgb, var(--dd-ink) 10%, var(--dd-paper-2))` (dark). Labels: paper-colored mono `font-size=7`, `letter-spacing=0.14em`, `text-anchor=middle`, centered at `chevron_cx, 21`.
...
Stroke: `var(--dd-wash-20)`, `stroke-width=0.8`, `stroke-dasharray=6,3`, `rx=6`. Zone fill: `var(--dd-wash-2)`.
...
Stroke: `color-mix(in srgb, var(--dd-ink) 18%, transparent)`, `stroke-width=1.2`, `rx=8`. Fill: `var(--dd-wash-2)`. K8s icon + label at `(cluster_x + 16, 352)` (icon) and `(cluster_x + 40, 362)` (text).
...
Stroke: `var(--dd-wash-20)`, `stroke-width=0.8`, `rx=6`. Fill: `var(--dd-wash-5)`. Icon at `(16, cross_y(k) + 10)`, name centered at `(effective_w / 2, cross_y(k) + 22)`, subtitle at `(effective_w / 2, cross_y(k) + 34)`.
...
Stroke: `color-mix(in srgb, var(--dd-ink) 18%, transparent)`, `stroke-width=0.8`, `rx=4`. Fill: `var(--dd-wash-5)`. Tool icon at the far right (`bar_x + bar_w - 50, 58`); name centered at `(bar_x + bar_w/2, 71)`; subtitle at `(bar_x + bar_w/2, 84)`.
...
**Focal node:** `fill="var(--dd-accent-tint)"`, `stroke="var(--dd-accent)"`, `stroke-width=1.2`. Title text in accent color. All other nodes: white fill, `stroke=color-mix(in srgb, var(--dd-ink) 25%, transparent)`, `stroke-width=1`.
...
Fills alternate `var(--dd-ink)` / `color-mix(in srgb, var(--dd-ink) 90%, var(--dd-paper))` (same palette as horizontals). Labels: paper-colored mono `font-size=7`, `letter-spacing=0.14em`, **rotated −90°**, anchored at `(strip_x + strip_w/2, (y0+y1)/2)`.
...
|---|---|---|---|---|---|
| `primary` | `var(--dd-accent)` | 1.2 | — | `arrow-accent` | Every edge whose endpoint is the `focal` node. |
| `secondary` | `var(--dd-muted)` | 1.0 | — | `arrow` | Default for source→component and component→component when neither endpoint is focal. |
| `trigger` | `var(--dd-muted)` | 1.0 | `4,3` | `arrow-sm` | Every edge originating from a `kind: bar` component. |
| `query` | `var(--dd-wash-30)` | 1.0 | `4,3` | `arrow` | Read-back edges (e.g., focal ↔ Trino). |

**Defs block** (required, exactly these four markers):

```svg
<defs>
  <marker id="arrow"        markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="var(--dd-muted)"/></marker>
  <marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="var(--dd-accent)"/></marker>
  <marker id="arrow-sm"     markerWidth="6" markerHeight="5" refX="5" refY="2.5" orient="auto"><polygon points="0 0, 6 2.5, 0 5" fill="var(--dd-muted)"/></marker>
  <marker id="arrow-dim"    markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="color-mix(in srgb, var(--dd-ink) 45%, transparent)"/></marker>
</defs>
```
...
| Token | Light | Dark |
|---|---|---|
| Page paper | `var(--dd-paper)` | `var(--dd-paper)` |
| Ink | `var(--dd-ink)` | `var(--dd-ink)` |
| Muted text | `var(--dd-muted)` | `color-mix(in srgb, var(--dd-ink) 65%, transparent)` |
| Chevron band A | `var(--dd-ink)` | `var(--dd-paper-2)` |
| Chevron band B | `color-mix(in srgb, var(--dd-ink) 90%, var(--dd-paper))` | `color-mix(in srgb, var(--dd-ink) 10%, var(--dd-paper-2))` |
| Chevron label | `var(--dd-paper)` | `var(--dd-ink)` |
| Dashed border | `var(--dd-wash-20)` | `color-mix(in srgb, var(--dd-ink) 22%, transparent)` |
| Cluster border | `color-mix(in srgb, var(--dd-ink) 18%, transparent)` | `color-mix(in srgb, var(--dd-ink) 18%, transparent)` |
| Node fill | white | `color-mix(in srgb, var(--dd-ink) 6%, transparent)` |
| Node stroke | `color-mix(in srgb, var(--dd-ink) 25%, transparent)` | `color-mix(in srgb, var(--dd-ink) 20%, transparent)` |
| Focal fill | `var(--dd-accent-tint)` | `color-mix(in srgb, var(--dd-accent) 12%, transparent)` |
| Focal stroke | `var(--dd-accent)` | `var(--dd-accent)` |
| Accent connector | `var(--dd-accent)` | `var(--dd-accent)` |
| Dot pattern | `color-mix(in srgb, var(--dd-ink) 10%, transparent)` | `color-mix(in srgb, var(--dd-ink) 10%, transparent)` |
