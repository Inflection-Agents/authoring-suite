...
**Selection triggers:** One central hub serves many interchangeable spokes, or many spokes feed
one hub — the spokes' *interchangeability*, not any individual spoke's identity, is the point.
Already drawable freehand in **Architecture**; this pattern is the grammar for it.

**Required primitives:** One hub, visually central. Spokes rendered visually identical to each
other — same size, same treatment — since their interchangeability is usually the whole claim.
Direction stated explicitly (does the hub serve the spokes, or do spokes feed the hub, or both):
distinguish with arrowheads or line style, not by making some spokes look different from others.
Where spokes exceed the budget, a grouped cohort with a stated count, never silent truncation.

**Complexity budget:** ≤8 spokes drawn individually; aggregate the rest into one cohort node
with a count. One hub. Radial layout or two flanking columns — pick whichever avoids connector
labels crossing each other at the actual spoke count; radial degrades first around 6–8 spokes.

**Anti-patterns:** Spokes varying in size or color, implying a hierarchy the claim never made.
A "hub" that actually has a distinct in-side and out-side — that's a pipeline, not a hub, and
belongs in Data flow or Architecture directly. Heavy fill on the hub: the geometry already
makes it central, so a heavy fill over-eggs it — border-only or a light tint reads as intended.
Radial layouts whose spoke labels cross their own connectors.

**Static fallback:** Show the hub and every spoke (or the cohort) in one frame, with direction
carried by arrowhead/line style rather than motion or sequencing.

**Nearest visual type:** **Architecture** — draw the hub as a focal node and the spokes as a
uniform ring or two flanking columns of otherwise-ordinary nodes; no new type is needed.
