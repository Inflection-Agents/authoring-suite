...
- **Trend line (optional):** `<line>` from lower-left to upper-right, stroke `color-mix(in srgb, var(--dd-ink) 25%, transparent)` dashed 4,3. Never force a perfect fit — only add if the trend is visually obvious.
...
<circle cx="X" cy="Y" r="5" fill="var(--dd-paper)"/>
<circle cx="X" cy="Y" r="5" fill="color-mix(in srgb, var(--dd-muted) 20%, transparent)" stroke="var(--dd-muted)" stroke-width="1"/>

<!-- Focal point -->
<circle cx="X" cy="Y" r="6" fill="var(--dd-paper)"/>
<circle cx="X" cy="Y" r="6" fill="color-mix(in srgb, var(--dd-accent) 15%, transparent)" stroke="var(--dd-accent)" stroke-width="1.2"/>
```
