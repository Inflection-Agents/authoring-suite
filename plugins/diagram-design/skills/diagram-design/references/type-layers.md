...
- Each row contains (left→right):
  ...
  2. **Layer name** slightly right of center-left — DM Sans 14–16px 600.
  3. **Sublabel / note** on the far right — Geist Mono 9–10px muted.
- Border between layers: 1px hairline `var(--dd-rule)`. Outer silhouette 1px ink or muted.
...
- Gold on **one** focal layer (stroke + subtle tint fill) — the bottleneck, the pay-rent layer, the one under discussion.
