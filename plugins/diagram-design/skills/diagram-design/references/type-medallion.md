...
|---|---|---|---|---|---|
| `outer` | `var(--dd-node-white)` | `muted` 1.0 solid | `muted @ 0.10` | `muted` | `muted` |
| `default` | `var(--dd-node-white)` | `ink` 1.0 solid | `ink @ 0.06` | `muted` | `muted` |
...
**Focal styling note:** the focal tier's accent treatment cascades — its bucket text and its example-value lines render in accent. Other field values (tool/format/writer) stay muted; only the bucket name and the example payload carry the focal signal so the tier card doesn't fully drown in gold.
...
| White (default card fill) | `var(--dd-node-white)` | `paper @ 0.04` |
