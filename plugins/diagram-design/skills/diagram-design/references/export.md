...
3. Make it standalone:
   ...
   - Inject Google Fonts `@import` so the SVG renders with correct typography in a browser. **XML-escape the `&` separators as `&amp;`** — a standalone `.svg` is parsed as strict XML, where a bare `&` starts an entity reference and makes the whole file fail to parse. (Don't copy the raw URL from the HTML `<link href>`; that ampersand form is only valid in HTML.)
     ```svg
     <defs>
       <style>@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,600;1,400&amp;family=DM+Sans:wght@300;400;500&amp;family=Geist+Mono:wght@400;500;600&amp;display=swap');</style>
     </defs>
     ```
     If the SVG already contains a `<defs>` block, **merge** the `<style>` into it (don't add a second `<defs>`).
   - **Inject the skin block into that same `<style>`.** The SVG body binds every colour and radius through `var(--dd-*)`, and those custom properties are declared on `:root` in the source HTML's `<style>` — which does not come along with the `<svg>` element. Without this step the exported file resolves nothing and renders unstyled: black fills on transparent ground, not a fallback to the intended palette.

     Copy the source's `:root { … }` declarations verbatim and rewrite the selector from `:root` to `svg`, so it applies in a standalone document where the SVG *is* the root:

     ```svg
     <defs>
       <style>
         @import url('https://fonts.googleapis.com/css2?…');
         svg { --dd-paper: #F5F2EC; --dd-ink: #1B2B3C; --dd-accent: #967A3A; /* … all 25 roles … */ }
     ...
     Carry **all 25 roles**, not the subset the diagram happens to use — the block is generated, and trimming it is how a later edit ends up referencing a token that is no longer declared. Carry the three radius rules too, or every corner falls back to its literal `rx` attribute and the geometry stops following the skin.
...
Two things a standalone `.svg` depends on the consumer to resolve:

- **Remote fonts.** Tools that don't fetch them at import time (offline Illustrator, some Figma import paths, older SVG viewers) will substitute typography.
- **CSS custom properties.** The palette rides `var(--dd-*)`, and the same class of consumer — older Illustrator, some Figma import paths, non-browser rasterizers — does not resolve custom properties. Those tools will not pick up the palette even though the skin block is present in the file.

The SVG renders correctly in any modern browser. For pixel-perfect portability, recommend the PNG export: it rasterizes the original HTML in Chromium, where both the fonts and the custom properties resolve natively, so **PNG export needs no skin handling at all**.
