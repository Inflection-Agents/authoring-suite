...
If the site has only one family, keep the schematic defaults for the missing roles (Cormorant Garamond for title, Geist Mono for mono). Don't force-pick a mono font that isn't on the site.
...
- **paper ≠ pure white**: if the site uses `var(--dd-node-white)`, fall back to `#fafaf7` to preserve Diagram Design's warm-neutral feel — or ask the user to confirm pure-white is intentional.
...
1. Open `assets/index.html` (gallery) and confirm the new palette feels coherent across all 32 types.
...
**From Markdown tables:** look for rows with hex values (`#rrggbb`) adjacent to role-like words. A row like `| accent | var(--dd-accent) |` maps directly.
...
```json
{ "colors": { "brand": { "primary": { "value": "var(--dd-accent)", "type": "color" } } } }
```
