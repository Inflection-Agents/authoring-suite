...
- 1–2 gold focal nodes: the primary integration point, the primary data store, or the key decision node.
...
<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="8"
      fill="var(--dd-wash-2)" stroke="color-mix(in srgb, var(--dd-ink) 10%, transparent)" stroke-width="0.8"/>
<rect x="{label_x}" y="{y+4}" width="{label_w}" height="12" rx="2" fill="{paper}"/>
<text x="{label_cx}" y="{y+13}" fill="color-mix(in srgb, var(--dd-ink) 40%, transparent)" font-size="7"
...
- Zone fill: `var(--dd-wash-2)` (2% ink wash). Any stronger competes with node fills.
...
## Anti-patterns
- Every box in gold ("this is important too") — hierarchy collapses.
