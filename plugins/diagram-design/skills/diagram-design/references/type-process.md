...
- Dot pattern: 22×22 grid, `circle r=0.8`, `fill color-mix(in srgb, var(--dd-ink) 10%, transparent)`. Opacity 0.55.
- Alternating lane tints: odd-indexed lanes (0, 2, …) receive `var(--dd-wash-2)` fill from `x=140` to `viewBox_w`.
- Lane dividers: horizontal hairlines at every `lane_y_top(k)` and at `legend_y_top`; stroke `var(--dd-rule)` width 0.8.
- Label column right border: vertical hairline at `x = label_col_w`, stroke `var(--dd-wash-20)` width 1, from `y = header_h` to `y = legend_y_top`.
...
- Default fill: `var(--dd-rule)`, number text ink.
- Focal fill: `color-mix(in srgb, var(--dd-accent) 20%, transparent)`, number text accent (§5).
...
- Default fill: muted (`var(--dd-muted)` light / `var(--dd-rule-solid)` dark).
- Focal fill: accent (`var(--dd-accent)` light / `var(--dd-accent)` dark).
...
|---|---|---|---|---|---|
| `normal` | `var(--dd-muted)` (muted) | 1.0 | — | `arrow` | Standard data hand-off between steps or actors. Unlabelled. |
| `focal-in` / `focal-out` | `var(--dd-accent)` (accent) | 1.2 | — | `arrow-accent` | Every edge whose endpoint is the focal node (`focal-in`) or origin is the focal node (`focal-out`). |
| `trigger` | `var(--dd-muted)` (muted) | 1.0 | `4,3` | `arrow-sm` | Orchestration trigger (scheduler → tool, manual override → upstream step). Unlabelled. |
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="11" cy="11" r="0.8" fill="color-mix(in srgb, var(--dd-ink) 10%, transparent)"/>
  </pattern>
...
| Token | Light | Dark |
|---|---|---|
| Paper | `var(--dd-paper)` | `var(--dd-paper)` |
| Ink | `var(--dd-ink)` | `var(--dd-ink)` |
| Muted | `var(--dd-muted)` | `var(--dd-muted)` |
| Soft | `var(--dd-soft)` | `var(--dd-soft)` |
...
| Lane tint | `var(--dd-wash-2)` | `color-mix(in srgb, var(--dd-ink) 2.5%, transparent)` |
| Dividers | `var(--dd-rule)` | `var(--dd-rule)` |
| Label col divider | `var(--dd-wash-20)` | `color-mix(in srgb, var(--dd-ink) 22%, transparent)` |
| Default chip fill | `var(--dd-rule)` | `var(--dd-rule)` |
| Focal chip fill | `color-mix(in srgb, var(--dd-accent) 20%, transparent)` | `color-mix(in srgb, var(--dd-accent) 22%, transparent)` |
| Default node fill | white | `color-mix(in srgb, var(--dd-ink) 4%, transparent)` |
| Default node stroke | `color-mix(in srgb, var(--dd-ink) 25%, transparent)` | `color-mix(in srgb, var(--dd-ink) 20%, transparent)` |
| Focal node fill | `var(--dd-accent-tint)` | `color-mix(in srgb, var(--dd-accent) 12%, transparent)` |
| Focal node stroke | `var(--dd-accent)` | `var(--dd-accent)` |
...
|------|---------------|--------------|---------|
| `LS` | `var(--dd-series-1)` sage | `var(--dd-series-1)` | List / assignment / task |
| `DB` | `var(--dd-series-2)` dusty-blue | `var(--dd-series-2)` | Dataset / tabular records |
| `TB` | `var(--dd-series-3)` mustard | `var(--dd-series-3)` | Table (analysis-ready) |
| `FL` | `var(--dd-series-4)` rust-brown | `var(--dd-series-4)` | File / document / report |
| `WB` | `var(--dd-series-5)` slate | `var(--dd-series-5)` | Web / press / public release |
