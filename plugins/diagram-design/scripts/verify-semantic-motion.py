...
def verify_markdown() -> list[str]:
    ...
    routing_position = skill.find("references/routing.md")
    if router_position < 0:
        errors.append("SKILL.md must link to semantic-patterns.md")
    if routing_position < 0:
        errors.append("SKILL.md must link to references/routing.md for the selection tables")
    if router_position >= 0 and routing_position >= 0 and router_position > routing_position:
        errors.append("semantic-pattern router must precede the routing.md pointer")
    if "### Visual-type guide (32)" not in routing:
        errors.append("routing.md must contain the 32-row visual-type guide")

    visual_guide = section(routing, "### Visual-type guide (32)", "Rules of thumb:")
    visual_rows = re.findall(r"^\|.*\[type-[^)]+\.md\]", visual_guide, re.MULTILINE)
    if len(visual_rows) != 32:
        errors.append(f"visual-type guide must preserve 32 rows; found {len(visual_rows)}")

    for index, name in enumerate(PATTERN_NAMES, 1):
        if name not in skill and name not in routing:
            errors.append(f"SKILL.md/routing.md does not route semantic pattern: {name}")
...
def main() -> int:
    ...
    if not args.example_only:
        print("OK: 8 semantic patterns route independently to the preserved 32 visual types")
