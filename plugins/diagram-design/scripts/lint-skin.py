...
NAMED_COLOR_ATTR_RE = re.compile(
    r'\b(?:fill|stroke)\s*=\s*"([a-zA-Z]+)"'
)
ALLOWED_NAMED_COLOR_VALUES = {"none", "transparent", "currentcolor"}
VAR_RE = re.compile(r"var\(\s*(--dd-[a-z0-9-]+)\s*[,)]")
TOKEN_DECL_RE = re.compile(r"(--dd-[a-z0-9-]+)\s*:")
...
def contract_roles():
    ...
    spec = importlib.util.spec_from_file_location(
        "diagram_design_build_skins", build_skins
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"could not load the token contract from {build_skins}")
...
def lint_text(
    ...
    for match in NAMED_COLOR_ATTR_RE.finditer(text):
        value = match.group(1)
        if value.casefold() not in ALLOWED_NAMED_COLOR_VALUES:
            add(
                match.start(),
                "color",
                f'CSS keyword color "{value}" bypasses the token contract — use '
                f"var(--dd-*) or a style-guide hex instead",
            )
    ...
    # Coverage: a file that binds through the contract must carry the whole skin block,
    # so a reskin stays a single :root swap and a half-written skin cannot ship.
    if referenced_tokens:
        declared = {
            match.group(1).removeprefix("--dd-") for match in TOKEN_DECL_RE.finditer(text)
        }
        undeclared = sorted(CONTRACT_ROLES - declared)
        if undeclared:
            add(
                0,
                "color",
                "skin block is incomplete, missing contract role(s): "
                + ", ".join(f"--dd-{role}" for role in undeclared),
            )
