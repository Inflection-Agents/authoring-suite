...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="verify-motion-") as temporary:
        ...
        original_routing = semantic_module.ROUTING
        try:
            missing_guide = directory / "missing-guide.md"
            missing_guide.write_text(
                original_routing.read_text(encoding="utf-8").replace(
                    "### Visual-type guide (32)", "### Visual guide"
                ),
                encoding="utf-8",
            )
            semantic_module.ROUTING = missing_guide
            markdown_errors = semantic_module.verify_markdown()
        finally:
            semantic_module.ROUTING = original_routing
        if not any("must contain the 32-row visual-type guide" in error for error in markdown_errors):
