...
def active_skin_block():
    """The generated light skin's declarations, indented for the gallery's :root."""
    source = (REPO / "skills/diagram-design/assets/skins/active-light.tokens.css").read_text(
        encoding="utf-8"
    )
    body = re.search(r":root\s*\{(?P<body>[^}]*)\}", source).group("body")
    return "\n".join(f"      {line.strip()}" for line in body.splitlines() if line.strip())


def style_guide_typography():
    """Return (font link, sans stack, serif stack, mono stack) from the style guide."""
    markdown = (REPO / "skills/diagram-design/references/style-guide.md").read_text(
        encoding="utf-8"
    )
    link = re.search(r'href="(https://fonts\.googleapis\.com/css2\?[^"]*)"', markdown).group(1)
    families = {}
    for role, cells in re.findall(r"^\| `([a-z-]+)` \| ([^|]+) \|", markdown, re.MULTILINE):
        families[role] = cells.strip().rstrip("*").strip().removesuffix(" *italic*").strip()
    serif = families.get("title", "Instrument Serif")
    sans = families.get("node-name", "Geist")
    mono = families.get("sublabel", "Geist Mono")
    return (
        link,
...
{SKIN}
    ...
    .cell:hover { border-color: var(--dd-accent); color: var(--dd-accent); }
    ...
    .cell.missing .icon { font-size: 24px; color: var(--dd-muted); }
    footer {
      margin-top: 3rem;
      padding-top: 1.5rem;
...
def main() -> int:
    ...
    font_link, font_sans, font_serif, font_mono = style_guide_typography()
    html = (
        GALLERY_TEMPLATE.replace("{SECTIONS}", "\n".join(gallery_chunks))
        .replace("{SKIN}", active_skin_block())
