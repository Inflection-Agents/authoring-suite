...
BUILD_SKINS = ROOT / "scripts/build-skins.py"
MOTION_TEMPLATE = ROOT / "skills/diagram-design/assets/template-motion.html"


def load_contract_roles() -> set[str]:
    """The token contract has exactly one definition — read it from the builder."""
    spec = importlib.util.spec_from_file_location("contract_build_skins", BUILD_SKINS)
...
CONTRACT_ROLES = load_contract_roles()
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="lint-a11y-") as temp_dir:
        ...
        named_color_svg = VALID_SVG.replace(
            "fixture-title", "named-color-title"
        ).replace("fixture-desc", "named-color-desc")
        require_lint_finding(
            "named-color-keyword",
            named_color_svg.replace(
                '<rect width="10" height="10"/>',
                '<rect width="10" height="10" fill="white"/>',
            ),
            'color: CSS keyword color "white" bypasses the token contract',
            directory,
        )
        allowed_keyword_svg = VALID_SVG.replace(
            "fixture-title", "allowed-color-keywords-title"
        ).replace("fixture-desc", "allowed-color-keywords-desc")
        require_pass(
            "allowed-color-keywords",
            allowed_keyword_svg.replace(
                '<rect width="10" height="10"/>',
        ...
        # --dd-* tokens must resolve against the contract in build-skins.py.
        skin_block = ":root{" + "".join(
            f"--dd-{role}: #f5f5f5;" for role in sorted(CONTRACT_ROLES)
        ) + "}"
        unknown_token_svg = VALID_SVG.replace(
            "fixture-title", "unknown-token-title"
        ).replace("fixture-desc", "unknown-token-desc")
        require_lint_finding(
            "unknown-token",
            f"<style>{skin_block}</style>\n"
            + unknown_token_svg.replace(
                '<rect width="10" height="10"/>',
                '<rect width="10" height="10" fill="var(--dd-nonesuch)"/>',
            ),
            "color: --dd-nonesuch is not a contract role",
            directory,
        )
        known_token_svg = VALID_SVG.replace(
            "fixture-title", "known-token-title"
        ).replace("fixture-desc", "known-token-desc")
        require_pass(
            "known-token",
            f"<style>{skin_block}</style>\n"
            + known_token_svg.replace(
                '<rect width="10" height="10"/>',
                '<rect width="10" height="10" fill="var(--dd-paper)" '
                'stroke="var(--dd-ink)" rx="var(--dd-radius-node)"/>',
            ),
            directory,
        )
        incomplete_skin = ":root{--dd-paper: #f5f5f5;--dd-ink: #2d3142;}"
        partial_svg = VALID_SVG.replace(
            "fixture-title", "partial-skin-title"
        ).replace("fixture-desc", "partial-skin-desc")
        require_lint_finding(
            "partial-skin",
            f"<style>{incomplete_skin}</style>\n"
            + partial_svg.replace(
                '<rect width="10" height="10"/>',
                '<rect width="10" height="10" fill="var(--dd-paper)"/>',
            ),
            "color: skin block is incomplete, missing contract role(s):",
            directory,
        )
        no_token_svg = VALID_SVG.replace("fixture-title", "no-token-title").replace(
            "fixture-desc", "no-token-desc"
        )
        require_pass(
            "no-token",
            no_token_svg.replace(
                '<rect width="10" height="10"/>',
        ...
        # lint-skin.py reads the token contract from build-skins.py at import time.
        shutil.copy2(BUILD_SKINS, project / "scripts/build-skins.py")
        shutil.copy2(
        ...
        # Swap whichever family the Typography table currently names for `title`, so this
        # case keeps testing the reader rather than a particular skin's font choice.
        style_text = project_style.read_text(encoding="utf-8")
        title_row = re.search(r"^\| `title` \| ([^|]+?) \|", style_text, re.MULTILINE)
        if title_row is None:
            raise AssertionError("style guide has no `title` typography row")
        project_style.write_text(
            style_text.replace(title_row.group(0), "| `title` | Inter Tight |", 1),
            encoding="utf-8",
        )
